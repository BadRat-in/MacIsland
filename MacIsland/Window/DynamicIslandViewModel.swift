...
class DynamicIslandViewModel: NSObject, ObservableObject {
    ...
    /// Height is intentionally an integer (was 32.5) so the rounded
    /// `clipShape` corners snap to whole-pixel boundaries. The .5
    /// fractional height produced subpixel-rendering artefacts at
    /// the bottom-leading curve (visible in screenshots) and a
    /// related "square block" residue at the corners when the notch
    /// dimmed to 0.3 opacity during idle.
    let notchChargingSize: CGSize = .init(width: 280, height: 32)
